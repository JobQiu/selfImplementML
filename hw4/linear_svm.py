...
"""
  ...
  h = np.matmul(X,theta)
  yh = y * h
  zeros_ = np.zeros((y*h).shape)
  J = C * np.sum(np.maximum(zeros_,1-yh)) + np.sum(np.square(theta))/2.0
  J = J * 1.0 / m
  
  grad = theta - C * np.matmul(y[yh<1],X[yh<1])
  grad = grad * 1.0 / m
...
def svm_loss_naive(theta, X, y, reg):
  ...
  for i in range(m):
      for j in range(K):
          if y[i]!=j:
              J += max(0,np.matmul(theta.T[j],X[i]) - np.matmul(theta.T[y[i]],X[i]) + delta)
